...
// https://github.com/cengizhanbasak/react-date-countdown-timer/blob/master/src/index.js
					...
					{units.map((unit, key) => {
						if (significance.indexOf(unit) !== -1) {
							return (
								<span key={key}>
									<span className={`${unit}`}>
										{this.dissect(this.state[unit], unit)}
									</span>{" "}
									{this.state[unit] <= 1 && locales[key]}
									{this.state[unit] > 1 && locales_plural[key]}
									{` `}
								</span>
							);
						} else return null;
